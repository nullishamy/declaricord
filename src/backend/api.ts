...
import { APIChannel, APIGuild, APIOverwrite, APIRole, ChannelType, Routes } from "discord-api-types/v10";
import { Category, GuildChannel, Role, RoleOverride } from "../util/schema.js";
import { AllDisabledPerms, bitfieldToString, stringToBitField } from "./permissions.js";
...
export abstract class API {
    ...
    abstract fetchGlobalChannels(): Promise<GuildChannel[]>
    abstract fetchRoles(): Promise<Role[]>
    abstract fetchCategories(): Promise<Category[]>

    abstract pushChannel(channel: GuildChannel): Promise<void>
...
export class APIImpl extends API {
    ...
    async fetchGlobalChannels(): Promise<GuildChannel[]> {
        const channels: APIChannel[] = this.fetchedChannels ?? await this.rest.get(Routes.guildChannels(this.guildId)) as APIChannel[]
            ...
            .map(c => {
                if (c.type === ChannelType.GuildText) {
                    return {
                        ...
                        overrides: this.roleOverwritesToOverrides(c.permission_overwrites ?? [], mappedRoles),
                    }
                }
    ...
    async fetchRoles(): Promise<Role[]> {
        const guild = this.fetchedGuild ?? await this.rest.get(Routes.guild(this.guildId)) as APIGuild
    ...
    async fetchCategories(): Promise<Category[]> {
        ...
        return Object.values(this.fetchedChannels.reduce((acc, val) => {
            if (val.type === ChannelType.GuildText || val.type === ChannelType.GuildVoice) {
                ...
                if (!acc[val.parent_id]) {
                    ...
                    const roleOverrides: RoleOverride[] = this.roleOverwritesToOverrides(parent.permission_overwrites ?? [], mappedRoles)
                ...
                if (val.type === ChannelType.GuildText) {
                    acc[val.parent_id].channels.push({
                        ...
                        overrides: this.roleOverwritesToOverrides(val.permission_overwrites ?? [], mappedRoles),
                    })
                }
    ...
    async pushChannel(channel: GuildChannel): Promise<void> {
    ...
    private roleOverwritesToOverrides(overwrites: APIOverwrite[], roleMapping: Record<string, APIRole>): RoleOverride[] {
