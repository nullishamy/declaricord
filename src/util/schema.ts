...
const OverrideArray = z.union([z.array(RoleOverride), z.object({})]).default([])
...
export const GuildChannel = z.discriminatedUnion('type', [VoiceChannel, TextChannel])
    .transform(data => {
        if (data.type === 'text') {
            const { id, comment, topic, nsfw, slowmode, overrides } = data
...
export const Category = z.object({
    ...
    overrides: OverrideArray.transform(data => !Array.isArray(data) ? [] : data),
})
